...
probing conditions.
The \emph{controlled geometry track} (G-track) operates on a NumPy-only toy model with
explicit PPMI--SVD embeddings and fully inspectable weight matrices.
...
No PPMI--SVD embedding was supplied to T5; no T5 weight was used in the G-track.
...
documented in \texttt{EXPERIMENT\_INDEX.md}.
...
The scripts for S-041 through S-057 are committed to the \texttt{S-Track} branch.
...
\subsection{G-Track: Controlled NumPy Geometry}

\paragraph{Design and independence.}
The G-track model is implemented entirely in NumPy and runs on CPU without GPU
or external framework dependencies.
It uses explicit PPMI--SVD embeddings computed from a small controlled vocabulary.
The full weight structure is inspectable: every matrix in the attention and FFN stack
is set directly rather than learned from data.
...
\end{itemize}
Scripts are committed to the \texttt{G-Track} branch.
...
The G-track used PPMI--SVD embeddings as a controlled reference geometry.
...
The comparison is structural and mechanistic, not a claim that the PPMI geometry
